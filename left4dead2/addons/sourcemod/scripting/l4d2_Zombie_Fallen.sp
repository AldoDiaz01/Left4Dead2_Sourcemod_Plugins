...
#define MELEE_RIOT_SHIELD "riot_shield"
...
new Handle: DropKnife = INVALID_HANDLE;
new Handle: DropRiot_shield = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	DropKnife = CreateConVar("Drop_Knife", "1.0", "Zombie Drop Knife Probability(0~100)", FCVAR_PLUGIN);
	DropRiot_shield = CreateConVar("DropRiot_Shield", "1.0", "Zombie Drop Riot_shield Probability(0~100)", FCVAR_PLUGIN);
...
public DropItem(Float:pos[3])
{
	...
	new KnifeChance = GetRandomInt(1,100);
	new Riot_shieldChance = GetRandomInt(1,100);
	...
	switch(GetRandomInt(0,34))
	{
		...
		case 2:
		{
					...
					new Clip_Size = GetEntProp(entity, Prop_Send, "m_iClip1");
				        SetEntProp(entity, Prop_Send, "m_iExtraPrimaryAmmo", Clip_Size * 2);
                                	return;
                            	}
                        }
                }
                         	...
                         	if (IsValidEntity(entity))
                            	{
	                           	...
	                           	DispatchSpawn(entity);
	                            	TeleportEntity(entity, pos, NULL_VECTOR, NULL_VECTOR);
					new Clip_Size = GetEntProp(entity, Prop_Send, "m_iClip1");
				        SetEntProp(entity, Prop_Send, "m_iExtraPrimaryAmmo", Clip_Size * 30);
                                	return;
                         	}
                        }
		...
		case 33:
		{
        	        if(KnifeChance <= GetConVarInt(DropKnife)) 
                  	{
                            	new entity = CreateEntityByName("weapon_melee");
                         	if (IsValidEntity(entity))
                            	{
		...
		case 34:
		{
        	        if(Riot_shieldChance <= GetConVarInt(DropRiot_shield)) 
                  	{
                            	new entity = CreateEntityByName("weapon_melee");
                         	if (IsValidEntity(entity))
                            	{
                            		pos[2] += 50.0;
                                        DispatchKeyValue(entity,"melee_script_name","riot_shield");
	                           	DispatchSpawn(entity);
	                            	TeleportEntity(entity, pos, NULL_VECTOR, NULL_VECTOR);
                                	return;
                         	}
