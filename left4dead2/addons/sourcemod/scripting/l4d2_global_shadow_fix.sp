...
*	Link	:	http://forums.alliedmods.net/showthread.php?t=149041
...
public Plugin myinfo =
{
	...
	url = "http://forums.alliedmods.net/showthread.php?t=149041"
}
...
{
	// Cvars
	CreateConVar("l4d2_global_shadow_fix_version", PLUGIN_VERSION, "Shadow Fix version.", FCVAR_SPONLY|FCVAR_NOTIFY|FCVAR_REPLICATED|FCVAR_DONTRECORD);
...
bool SetPos()
{
	...
	// Dead Center:
	if( StrEqual(sMap, "c1m1_hotel", false) )		//-0.612372 0.353553 -0.707106
		fCorrected = view_as<float>({ 0.587627, 0.353553, -0.207106 });

	else if( StrEqual(sMap, "c1m2_streets") )		//0.150383 0.086824 -0.984807
		fCorrected = view_as<float>({ 0.750383, 0.486824, -0.484807 });

	else if( StrEqual(sMap, "c1m3_mall") )			//01.187627 0.553553 -0.707106
		fCorrected = view_as<float>({ 2.587627, 0.953553, -0.207106 });

	else if( StrEqual(sMap, "c1m4_atrium") )		//-0.612372 0.353553 -0.707106
		fCorrected = view_as<float>({ 2.587627, 0.353553, -0.207106 });

	// Hard Rain:
	else if( StrEqual(sMap, "c4m1_milltown_a") )	//0.330366 -0.088521 -0.939692
